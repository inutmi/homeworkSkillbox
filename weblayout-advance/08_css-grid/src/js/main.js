...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const headerInp = document.querySelector('.header__inp_tablet');
  const headerClose = document.querySelector('.header__close');

  headerBtn.addEventListener('click', function () {
    headerInp.classList.add('header__inp_tablet_active');
  ...
  headerClose.addEventListener('click', function () {
    ...
    headerInp.classList.remove('header__inp_tablet_active');
  });

  setTransitionendListenter(headerInp, 'header__inp_tablet_active', 'block');
...
// удаление бургер-меню при увеличении размера окна
  window.addEventListener('resize', function() {if (window.innerWidth > 463) {burger.classList.remove('burger_active');
}
  });

  setTransitionendListenter(width, burger, 'burger_active', 'block');
...
const photo1 = document.querySelector('.pic');
const photo2 = document.querySelector('.works__event');
const photo3 = document.querySelector('.pic1');

function start() {
	let hElem2 = window.getComputedStyle(photo2, false).height;
  let hElem3 = window.getComputedStyle(photo3, false).height;
	hElem2 = Number(hElem2.slice(0, hElem2.length-2));
	photo1.style.height = (hElem2 - Number(hElem3.slice(0, hElem3.length-2))) + 'px';
}
